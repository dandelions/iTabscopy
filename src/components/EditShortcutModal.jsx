...
const EditShortcutModal = ({ isOpen, onClose, shortcut, onSave }) => {
    ...
    useEffect(() => {
        if (shortcut) {
            // 检查是否为文件夹
            const isFolderType = shortcut.type === 'folder';
    ...
    const handleSubmit = (e) => {
        ...
        if (isFolder) {
            // 如果没有选择图标，默认使用字母图标
            if (!finalIcon) {
                const firstChar = finalTitle || 'F';
                finalIcon = {
                    type: 'letter',
                    letter: firstChar[0].toUpperCase()
                };
            }
            
            onSave({
                ...shortcut,
                title: finalTitle || '文件夹',
    ...
    return (
        <div className="fixed inset-0 z-[10002] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-md bg-white/10 backdrop-blur-2xl border border-white/20 rounded-2xl shadow-2xl overflow-hidden" style={{ zIndex: 10003 }}>
                {/* Header */}
