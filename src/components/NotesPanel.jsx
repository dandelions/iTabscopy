...
import { X, Plus, Trash2, Check, Download, Upload, Search, StickyNote, ClipboardList, Circle, CheckCircle2 } from 'lucide-react';
  ...
  onOpenChange,
    ...
    return (
        <div className={`fixed inset-0 z-40 transition-all duration-300 ${isOpen ? 'visible' : 'invisible'}`}>
            ...
            />
            <div
                className={`absolute top-0 right-0 h-full max-w-full w-full md:w-[920px] bg-white/10 backdrop-blur-2xl border-l border-white/15 shadow-2xl flex rounded-l-2xl overflow-hidden transform transition-transform duration-300 ease-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
            >
                ...
                {activeSection === 'notes' ? (
                    ...
                    <div className="flex-1 flex flex-col">
                        <div className="flex items-center justify-between px-5 py-4 border-b border-white/10 bg-white/5">
                            <div>
                                <div className="text-white font-semibold text-lg">待办列表</div>
                                <div className="text-[11px] text-white/40">
                        ...
                        <div className="flex-1 overflow-auto p-5" onWheel={(e) => e.stopPropagation()}>
                            <form onSubmit={handleTodoSubmit} className="mb-5 rounded-2xl border border-white/10 bg-white/5 p-4">
                                <div className="flex gap-3">
                                    ...
                                    <button
                                        type="submit"
                                        className="flex items-center gap-2 rounded-xl bg-white/10 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-white/20 active:scale-95"
                                    >
                                        <Plus className="h-4 w-4" />
                                        添加
                                    </button>
